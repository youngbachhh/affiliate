...
const headers = [
  ...
  { title: "Mã khách hàng", key: "customerId" },
  { title: "Đơn hàng", key: "orders" },
  { title: "Tổng đã tiêu", key: "totalSpent" },
];
...
const { data: customerData } = await useApi<any>(
      ...
      >
        <template #item.customer="{ item }">
          <div class="d-flex align-center gap-x-3">
            <VAvatar size="34" :image="item.avatar" />
        ...
        <template #item.customerId="{ item }">
          <h6 class="text-h6 font-weight-regular">#{{ item.customerId }}</h6>
        </template>
        ...
        <template #item.country="{ item }">
          <div class="d-flex gap-x-2">
            <img :src="item.countryFlag" height="22" width="22" />
            <span class="text-body-1">{{ item.country }}</span>
          </div>
        </template>

        <template #item.totalSpent="{ item }">
          <h6 class="text-h6">${{ item.totalSpent }}</h6>
        </template>
          ...
          <div class="d-flex justify-end flex-wrap gap-x-6 px-2 py-1">
            ...
            <div class="d-flex gap-x-2 align-center me-2">
              ...
              <VBtn
                ...
                @click="
                  page >= Math.ceil(totalCustomers / itemsPerPage)
                    ? (page = Math.ceil(totalCustomers / itemsPerPage))
                    : page++
                "
              />
