...
const { data: vehiclesData } = await useApi<any>(
  ...
  <VCard>
    ...
    >
      <template #item.location="{ item }">
        <div class="py-2">
          <VAvatar variant="tonal" class="me-4" color="secondary">
            <VIcon icon="ri-bus-line" size="28" />
          </VAvatar>
          <RouterLink
      ...
      <template #item.warnings="{ item }">
        <VChip :color="resolveChipColor(item.warnings)" size="small">
          {{ item.warnings }}
        </VChip>
      </template>
        ...
        <div class="d-flex justify-end flex-wrap gap-x-6 px-2 py-1">
          ...
          <div class="d-flex gap-x-2 align-center me-2">
            ...
            <VBtn
              ...
              @click="
                page >= Math.ceil(totalVehicles / itemsPerPage)
                  ? (page = Math.ceil(totalVehicles / itemsPerPage))
                  : page++
              "
            />
